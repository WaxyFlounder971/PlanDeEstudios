...
function inicializarModalCompletarUniversidades() {
  document.getElementById("btn-guardar-completar-universidades").addEventListener("click", async () => {
    ...
    try {
      for (const fila of filas) {
        ...
        if (!plan) continue;
        const revisada = await confirmarUniversidadNoInvertida({
          siglas: fila.querySelector(".input-completar-siglas").value,
          nombre_completo: fila.querySelector(".input-completar-nombre").value,
          nombrePlan: plan.nombre_carrera,
        });
        resueltas.push({ plan, revisada });
      }
    } finally {
      btnGuardar.disabled = false;
    }

    resueltas.forEach(({ plan, revisada }) => {
      plan.universidad.nombre_completo = revisada.nombre_completo;
      plan.universidad.siglas = revisada.siglas;
