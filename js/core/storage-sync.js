...
async function manejarFalloReconexion() {
  ...
  // fallaba un intento, incluso cuando en realidad se estaba a mitad de
  ...
  // confirmado), sin tocar la sesión ni forzar nada. El próximo ciclo
  // automático (9s/45s/"online") vuelve a intentarlo solo.
  console.warn("Fallo al reconectar con conexión real confirmada. Se seguirá reintentando solo, sin cerrar la sesión.");
}
...
function manejarFalloDeRed(e) {
  if (e && e.reconexionFallida) {
    ...
    // FIX 2026-09-27: un fallo de red plano ya no salta directo a gris
    // ("desconectado"). Si el navegador todavía dice que hay red
    // (navigator.onLine !== false), es más honesto mostrar amarillo
    // ("reconectando" - un corte pasajero, Drive lento, lo que sea) que
    // gris (que se lee como "confirmado, no hay nada que hacer"). Solo se
    // muestra gris cuando el propio navegador confirma que no hay señal.
    mostrarIndicadorConexion(navigator.onLine === false ? "desconectado" : "reconectando");
  }
}
...
function inicializarReconexionAlVolverOnline() {
  ...
  window.addEventListener("online", () => {
    // FIX 2026-09-27 (reporte real: "me marcó como sin internet solamente
    // cuando conecté el internet, cuando estaba conectando"): el navegador
    // dispara "online" apenas detecta ALGUNA interfaz de red activa, no
    // cuando la conexión ya está totalmente estable - los primeros
    // intentos justo después de este evento son los que más fácil fallan
    // (DNS todavía resolviendo, handshake a medias). Antes ese primer
    // fallo transitorio prendía directo el gris de "sin conexión", que es
    // justo lo contrario de lo que estaba pasando (se estaba reconectando,
    // no desconectando). Ahora este evento muestra amarillo
    // ("reconectando") de una, y ese amarillo se resuelve solo a verde en
  ...
  // ninguna llamada a Drive llegue a fallar primero. Este es el único caso
...
/**
 * FIX 2026-09-27 (pedido explícito: "Sin conexión / Reconectando /
 * Conectado", gris/amarillo/verde, y que el verde desaparezca solo a los
 * ~3s): estado.conexionDrive ahora tiene 4 valores posibles:
 *   "ok"           -> nada que avisar, la píldora queda oculta.
 *   "desconectado" -> gris, se queda prendido mientras siga confirmado
 *                     que no hay conexión real (evento "offline", o un
 *                     fallo con probarConexionReal() negativo).
 *   "reconectando" -> amarillo, transitorio: se está reintentando ahora
 *                     mismo (evento "online" recién disparado, o un fallo
 *                     de red con el navegador todavía diciendo que hay
 *                     señal - ver manejarFalloDeRed/manejarFalloReconexion
 *                     más arriba).
 *   "conectado"    -> verde, transitorio: se acaba de recuperar la
 *                     conexión. Se apaga solo (setTimeout de 3s) y vuelve
 *                     a "ok" sin que el usuario tenga que tocar nada.
 *
 * La píldora es deliberadamente texto plano de color (mismo criterio
 * visual que el indicador de Tiempo de Estudio en la topbar), no un chip
 * ni un badge con fondo - "algo discreto", pedido explícito.
 */
const COLOR_CONEXION = {
  desconectado: "var(--text-secondary, #9ca3af)", // gris
  reconectando: "var(--color-warning, #f5a524)", // amarillo
  conectado: "var(--color-success, #22c55e)", // verde
};
...
function pintarPillConexion(nuevoEstado) {
  ...
  pill.textContent = TEXTO_CONEXION[nuevoEstado] || "";
  pill.style.color = COLOR_CONEXION[nuevoEstado] || "";
  pill.style.background = "transparent"; // discreto: sin chip, solo texto de color
  pill.style.cursor = nuevoEstado === "desconectado" ? "pointer" : "default";
}

let temporizadorOcultarConectado = null;

/** Punto único que cambia de estado real: pinta la píldora, actualiza el
 *  indicador del sidebar, y si el nuevo estado es "conectado" programa su
 *  propio apagado a los 3s. */
function mostrarIndicadorConexion(nuevoEstado) {
  clearTimeout(temporizadorOcultarConectado);
  estado.conexionDrive = nuevoEstado;
  pintarPillConexion(nuevoEstado);
  actualizarIndicadorSync();

  if (nuevoEstado === "conectado") {
    temporizadorOcultarConectado = setTimeout(() => {
      estado.conexionDrive = "ok";
      pintarPillConexion("ok");
      actualizarIndicadorSync();
    }, 3000);
  }
}

/** Compatibilidad: quien llama a esto no distingue "recién falló,
 *  reintentando" de "confirmado sin conexión" - cae directo en gris. Los
 *  caminos que sí saben distinguir (online/offline, manejarFalloDeRed)
 *  llaman a mostrarIndicadorConexion() de forma directa con el estado
 *  correcto. */
function mostrarAvisoReconexion() {
  mostrarIndicadorConexion("desconectado");
}

/** Se llama a mano (storage.js, tras establecer un token válido) o desde
 *  confirmarConexionOk() apenas cualquier llamada a Drive vuelve con
 *  éxito. Si venía de un estado con problema (gris o amarillo), flashea
 *  verde "Conectado" 3s y se apaga solo; si ya estaba todo bien, no hay
...
function ocultarAvisoReconexion() {
  ...
  const veniaDeUnProblema = estado.conexionDrive === "desconectado" || estado.conexionDrive === "reconectando";
  if (veniaDeUnProblema) {
    mostrarIndicadorConexion("conectado");
  } else {
    clearTimeout(temporizadorOcultarConectado);
    estado.conexionDrive = "ok";
    pintarPillConexion("ok");
...
/**
 * Botón 🔄 / mantener presionado el indicador: fuerza el intento de
 * sincronización YA, sin esperar el evento "online" del navegador (útil si
 * la conexión volvió pero el evento no disparó, o si se quiere forzar un
 * guardado inmediato).
 *
 * FIX 2026-09-27 (reporte real: "cuando le doy recargar me sale ya estaba
 * cargado y no tiene sentido, no tiene wifi"): antes, si no había cambios
 * LOCALES pendientes, esta función ni siquiera intentaba hablar con
 * Drive - declaraba "Ya estaba sincronizado" a ciegas, sin haber hecho ni
 * un solo request. Por eso mentía sin wifi: no hay forma honesta de decir
 * "ya está todo al día" sin haber preguntado. Ahora siempre pasa por
 * sincronizarAhora() (sube lo pendiente si lo hay, y de cualquier forma
 * baja/confirma contra Drive), que sí habla con la red de verdad y ya sabe
 * reflejar el resultado con honestidad (toast + indicador gris/amarillo/
...
function actualizarBadgeConflictosGlobales() {
  ...
  badge.title = n === 1 ? "1 choque pendiente de resolver. Toca para verlo." : `${n} choques pendientes de resolver. Toca para verlos.`;
  badge.onclick = () => abrirModalTodosLosConflictos();
}

/**
 * FIX 2026-09-27 (pedido explícito): el sidebar (≥900px) ahora refleja los
 * mismos 3 estados de conexión que la píldora de móvil (gris/amarillo/
 * verde, ver mostrarIndicadorConexion más arriba), en vez del viejo rojo
 * fijo para "sin conexión". Sigue priorizando el estado de conexión sobre
 * "cambios sin sincronizar" - si Drive no responde, eso es lo más
 * importante que el usuario necesita saber, tenga o no cambios pendientes.
 * El texto ya no lleva la instrucción "toca para reconectar" pegada (se
 * movió al title, como tooltip) para que se lea corto y discreto.
 */
function actualizarIndicadorSync() {
  ...
  if (estado.conexionDrive === "desconectado") {
    ...
    el.title = "Sin conexión con Drive. Toca para reconectar.";
    el.className = "badge badge-neutral";
    el.style.cursor = "pointer";
  } else if (estado.conexionDrive === "reconectando") {
    el.textContent = "Reconectando";
    el.title = "Reconectando con Drive...";
    el.className = "badge badge-warning";
    el.style.cursor = "";
  } else if (estado.conexionDrive === "conectado") {
