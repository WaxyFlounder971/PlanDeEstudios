...
Propósito: **(nuevo, 2026-09-09, "Competencias" Parte 1/2)** una "competencia" es un grupo de horas-de-estudio compartido entre varias personas — a diferencia de TODO lo demás en la app, no vive en el Drive de cada usuario: vive del lado del Worker (`worker-notificaciones-agenda`, mismo que ya usa `core/auth.js` para OAuth). Lo único que sincroniza por Drive (como el resto de `estado.datos`) es la lista LOCAL de "a qué competencias estoy unido" (`estado.datos.competencias_unidas`, ver core/schema.js) — un puntero por competencia (id + mi apodo ahí + mi participante_id), no el contenido de la competencia en sí. Parte 1 (esta ronda) cubre crear/unirse/ver lista/copiar invitación/salir; Parte 2 (pendiente) es la tabla de posiciones real y el envío periódico de horas trabajadas.
Depende de: core/storage.js, core/schema.js (`sellarTimestamp`), core/storage-sync.js (`marcarCambioPendiente`), core/auth.js (`URL_WORKER_OAUTH`, exportada recién en esta ronda — antes era privada del archivo), ui/componentes.js (`mostrarToast`, `abrirConfirmacion`), core/clipboard.js (`copiarAlPortapapelesBlindado`, `abrirModalCopiaManualPortapapeles`)
Exporta:
* `construirVistaCompetencias(cont, refrescar)` — punto de entrada, llamado desde `tiempo-estudio.js` cuando el pill superior (ahora de 3, no 2) está en "Competencias". Lista las competencias de `estado.datos.competencias_unidas` como tarjetas (apodo propio, 👑 si `es_creador`), cada una con "Copiar invitación" y "Dejar de ver" (🗑️); estado vacío con botones "Crear"/"Unirse" si no hay ninguna.
* `abrirModalCrearCompetencia(refrescar)` *(interna)* — pide nombre + apodo, `POST /competencias/crear` (Worker), guarda la entrada en `competencias_unidas` (`sellarTimestamp`) y el `tokenCreador` de la respuesta en `localStorage` (nunca en `estado.datos` — mismo criterio que `dispositivo_id`: es un secreto de "solo quien creó puede borrar", no necesita viajar sincronizado). Al terminar bien, ofrece copiar el link de invitación de una.
* `abrirModalUnirseCompetencia(refrescar)` *(interna)* — acepta pegar el link completo o el id pelado (`extraerIdCompetenciaDeTexto`, parsea `?comp=<id>` de una URL si corresponde), pide apodo, `POST /competencias/:id/unirse`. El `nombre` de la competencia SIEMPRE lo devuelve el Worker en la respuesta — nunca se inventa del lado del cliente.
* `salirDeCompetencia(competencia, refrescar)` *(interna)* — solo deja de ver la competencia en este dispositivo (tumba en `_eliminados_competencias_unidas`, mismo patrón `{id, eliminadoEn}` correcto que el fix de `eliminarSesion` de esta misma ronda) — NO borra la competencia del lado del Worker ni afecta a los demás participantes; avisa distinto si `es_creador` ("esto no la borra para los demás").
* `copiarLinkInvitacion(competencia)` *(interna)* — mismo blindaje de 2 capas que ya usa el flujo "Enviar a Claude" (`core/clipboard.js`): si la copia automática falla por cualquier motivo, abre el modal de copia manual en vez de dejar a la persona sin ninguna pista.

**⚠️ Contrato con el Worker — todavía NO implementado del lado servidor** (repo aparte, no incluido en este proyecto): `worker-notificaciones-agenda` hoy solo tiene `/oauth/exchange` y `/oauth/refresh`. Hacen falta 2 rutas nuevas antes de que "Crear"/"Unirse" funcionen de verdad:
```
POST /competencias/crear          { nombre, apodo, correo } → 201 { id, tokenCreador, participanteId }
POST /competencias/:id/unirse     { apodo, correo }         → 200 { participanteId, nombre } · 404 si no existe
```
Ambas con header `Authorization: Bearer <estado.token>` (mismo access_token de Google que ya usa el resto de la app). Hasta que existan, ambos modales fallan con un toast de error manejado (no rompe nada) — UI, datos y sync ya están terminados y listos para usarlas apenas el backend esté.
