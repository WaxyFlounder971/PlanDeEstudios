...
* `crearMateriaMatriculada({ materiaId, planEstudioId })` — matricula una materia real dentro de un semestre. Incluye `tiempo_estudio: { meta_horas_semana, pomodoro, color, ultima_semana_felicitada }` (Tiempo de Estudio, Parte 1 + Parte 3): `ultima_semana_felicitada` (Parte 3, 2026-09-05) guarda el epoch ms del lunes de la última semana en la que ya se felicitó a esta materia por cruzar su meta — evita repetir la felicitación en cada apertura de la app la misma semana, ver `revisarFelicitacionMeta()` en `tiempo-estudio-timer.js`.
...
* `eliminarSesion(sesion, refrescar)` *(interna)* — confirma con `abrirConfirmacion`, luego saca la sesión del arreglo vivo y agrega su `id` a la tumba `estado.datos._eliminados_sesiones_estudio` (mismo patrón "Borrado = tumba" del resto de la app), antes de `marcarCambioPendiente()`.
...
Depende de: core/storage.js, core/utils.js (`aplicarFormatoTexto`), core/schema.js (`COLOR_TIEMPO_ESTUDIO_DEFAULT`)
Exporta:
...
* `construirEstadisticasMateria(cont, mm, color, refrescar)` — **(2026-09-07, nuevo, pedido explícito)** a diferencia de `construirVistaEstadisticas` (agregado de TODAS las materias, vive en el pill superior "Estadísticas"), esto se llama desde `construirPantallaDetalle` de `tiempo-estudio.js`, DENTRO del detalle de una materia puntual, siempre filtrado por `materia_matriculada_id` — cada repetición de una materia tiene sus propias gráficas. `color` es el mismo color efectivo que ya resuelve `obtenerColorMateria()` en `tiempo-estudio.js` (propio > categoría > default), recibido por parámetro para no duplicar esa cadena de fallbacks acá. Arma, en orden: 1) `construirSeccionResumenMetas` — gráfica de líneas (línea nueva `construirGraficaLineas`, generaliza a N series el mismo sistema de coordenadas de `construirGraficaBarras`) con el trabajado de cada día de ESTA semana en el color propio de la materia, contra una línea gris punteada de la meta diaria pareja (`meta_horas_semana / 7`, mismo reparto que `obtenerEstudioParaHoy()` de `tiempo-estudio.js`), más 2 contadores debajo con el total semanal de cada línea; si la materia no tiene meta configurada, muestra un mensaje en vez de la gráfica. 2) `construirSeccionBarrasMateria` — mismo pill Semana/Semestre y navegador `‹ ›` que la tendencia global, pero filtrada a esta materia (`calcularMinutosMateriaEnRango`) y con las barras pintadas del color propio de la materia (antes solo existía en violeta genérico, agregado de todas las materias). 3) `construirSeccionResumenFinal` — 4 totales de siempre (todo el historial, sin filtrar por período): horas totales estudiadas, día más productivo (agrupa sesiones por fecha local y toma el máximo), sesiones totales, sesión promedio.
