...
* **ASUNCIÓN A CONFIRMAR:** el nombre de la cuenta de Google se lee de `estado.perfil.nombre` (dato de sesión vivo, igual que `estado.token` — no viene de `estado.datos`, que es el JSON sincronizado a Drive). No se tuvo `main.js` a la vista para confirmar dónde guarda realmente el resultado de `obtenerPerfilGoogle` (`auth.js`) — si el campo real es otro, solo hay que ajustar `obtenerNombrePerfilGoogleCrudo()`, el resto de esta sección no depende de eso.
