...
* `resolverMateriaVinculada(nombreMateria)` — match EXACTO contra `obtenerMateriasVinculablesAgenda()` primero (como siempre); si no hay exacto, fallback normalizado (sin acentos, minúsculas, espacios colapsados) SOLO si resuelve a una única materia sin ambigüedad — cubre diferencias menores entre lo que Gemini devuelve y el nombre guardado (espacio de más, acento, mayúscula). No resuelve abreviaturas propias del usuario tipo "AP2" que no coincidan de cerca con el nombre oficial ni tengan un apodo guardado en Horario — para eso sigue siendo necesario poner el apodo real en Horario (`construirMapaApodosMaterias`/campo `apodo` de un bloque), el único mecanismo pensado para abreviaturas arbitrarias.
...
* `CAPACIDADES_WAPPER` (2026-08-31, punto 7 del brief) — arreglo mantenible `{descripcion, ejemplo}[]` con las capacidades REALES ya implementadas de Wapper, en orden lógico (crear → consultar tareas/exámenes → buscar ítem puntual → consultar modalidad → cambiar modalidad → cambiar apodo; las dos entradas de modalidad quedan juntas a propósito, ajuste 2026-08-31). `ejemplo` es normalmente un string fijo, salvo en "Cambiar cómo me dirijo a ti" donde es una FUNCIÓN — se resuelve al vuelo en cada llamada a `construirMensajeCapacidadesWapper` para mostrar un apodo al azar de `APODOS_EJEMPLO_CAPACIDADES` en vez de repetir siempre el mismo. Agregar una capacidad nueva en el futuro (ej. Tiempo de Estudio) es sumar una entrada acá — no hay texto de la lista enterrado en ningún prompt de Gemini ni hardcodeado aparte.
* `APODOS_EJEMPLO_CAPACIDADES` (2026-08-31) — 22 apodos de ejemplo (Fer, Pipo, Juanjo, Male, Vale, Santi, Naty, Pao, Andy, Nacho, Kike, Meli, Fabi, Caro, Tavo, Rodri, Gaby, Chepe, Mafe, Toño, Vicky, Lalo) usados SOLO para el ejemplo de "actualizar_nombre" en la lista de capacidades — no tiene relación con `asistente_nombre_preferido` real del usuario ni con `resolverActualizacionNombre`.
* `construirMensajeCapacidadesWapper()` — arma el texto "Puedo ayudarte con: • descripcion.\nEjemplo: "..."" (viñeta "•", ejemplo en su propia línea debajo de la descripción — ajuste 2026-08-31) recorriendo `CAPACIDADES_WAPPER` en vivo (nunca cacheado); si `c.ejemplo` es función la ejecuta en el momento (ver `APODOS_EJEMPLO_CAPACIDADES`), mismo principio que `construirEjemplosBienvenida`.
