...
* `CAPACIDADES_WAPPER` (2026-08-31, punto 7 del brief) — arreglo mantenible `{descripcion, ejemplo}[]` con las capacidades REALES ya implementadas de Wapper (crear tarea/examen/evento, editar modalidad, consultar tareas/exámenes de una semana, buscar un ítem puntual, consultar modalidad de una clase, cambiar apodo). Agregar una capacidad nueva en el futuro (ej. Tiempo de Estudio) es sumar una entrada acá — no hay texto de la lista enterrado en ningún prompt de Gemini ni hardcodeado aparte.
* `construirMensajeCapacidadesWapper()` — arma el texto "Puedo ayudarte con: * descripcion. Ejemplo: "..."" recorriendo `CAPACIDADES_WAPPER` en vivo (nunca cacheado), mismo principio que `construirEjemplosBienvenida`.
