...
> **Estado:** 48/49+ archivos `.js` del proyecto documentados (se sumó `asistente/asistente.js`, 2026-08-29; actualizado 2026-08-31 con la lista de capacidades del punto 7 del brief; 2026-09-04 se sumó el grupo completo `js/tiempo-estudio/` — `tiempo-estudio.js`, `tiempo-estudio-config.js`, `tiempo-estudio-timer.js` — tras la Parte 2 del motor de Pomodoro/salvavidas; 2026-09-05 se sumaron `tiempo-estudio-registro.js` y `tiempo-estudio-estadisticas.js` tras la Parte 3, registro manual/estadísticas/felicitación; 2026-09-08/09 — ronda de bugfixes + "Entrega 3": fix de tumba mal formada en `eliminarSesion` (tiempo-estudio-registro.js, el borrado de sesiones no se mantenía guardado), fix de TDZ por import circular en 3 hooks de `storage-sync.js`, "Semana N" pasó de ISO-del-año a semana-del-semestre real (`agenda-clases.js#calcularNumeroSemanaParaFecha`) en los 4 navegadores de tiempo-estudio-estadisticas.js, nuevo campo `dias_estudio` por materia (schema.js + picker en tiempo-estudio-config.js) y reparto real de la meta diaria (`calcularMetaDiariaMateria`, reemplaza el `/7` parejo tanto en "Resumen de metas" como en `obtenerEstudioParaHoy()`/Agenda); **2026-09-09 — se sumó `js/tiempo-estudio/tiempo-estudio-competencias.js`** (Competencias, Parte 1+2, contrato reescrito de cero contra el Worker REAL — la ronda anterior había adivinado uno equivocado antes de tener `worker-notificaciones-agenda/src/index.js` a la vista), nueva colección `competencias_unidas` en schema.js + storage-merge.js, `URL_WORKER_OAUTH` exportada de auth.js, envío de horas enganchado en 6 puntos (tiempo-estudio-timer.js x3, tiempo-estudio-registro.js x3), y Tiempo de Estudio se sumó a `SECCIONES_TOGGLEABLES` (config-ajustes.js) — confirmado con main.js/index.html que el id `"tiempo-estudio"` y `DEFAULT_ORDEN_NAV` ya coinciden, sin nada más pendiente ahí. **1 pendiente detectado en esta ronda:** `asistente/asistente-bandeja.js` — existe (lo referencian los comentarios de exports de `asistente.js`, ronda "Bandeja pendiente / Captura por voz") pero nunca se subió/documentó acá; no se documenta en esta ronda por no tener el archivo a la vista, para no inventar su lista de exports.

**2026-09-17 — Ronda de bugs del timer + Competencias (Partes 1 a 5 del prompt).** **Bug real de pérdida de tiempo, resuelto:** `revisarSesionOlvidadaAlAbrir()` (tiempo-estudio-timer.js) nunca volvía a levantar el timer en memoria al reabrir la app — sus 3 caminos terminaban en `localStorage.removeItem()`, así que cerrar/recargar la pestaña mataba la sesión activa en silencio; ahora existe `restaurarTimerDesdeSnapshot()` y el salvavidas dejó de ser destructivo. Además: avisos de fin de bloque que sí llegan con la app en 2do plano (Service Worker + alarma de fase), `saltarDescansoPomodoro()`, "Tiempo de Estudio" → "Tiempo" en la interfaz, ningún modal de la sección se cierra al tocar fuera, y 2 archivos nuevos — `tiempo-estudio-celebracion.js` (confeti/audio/aviso previo) y `tiempo-estudio-competencias-gestion.js` (menú Gestionar + Registro de finalizadas, separado porque `tiempo-estudio-competencias.js` ya pasaba las 1100 líneas). Del lado del Worker: 4 endpoints nuevos + 2 columnas D1 (`competencias.estado`, `participantes.horas_totales_historicas`, ver `migracion_competencias_gestion.sql`). **Pendiente detectado:** el grupo `js/tiempo-estudio/` pasó de 5 a 7 archivos y `tiempo-estudio-estadisticas.js` sigue sin subirse a estas rondas (se documenta de memoria, no se tocó).
...
- [`js/tiempo-estudio/`](#js--tiempo-de-estudio) — Tiempo de Estudio (7 archivos; la sección se llama "Tiempo" en la interfaz desde 2026-09-17, los archivos NO se renombraron para no romper imports)
...
* `crearSesionEstudio({ materiaMatriculadaId, inicio, fin, origen })` — crea una `sesion_estudio` ya cerrada (con inicio Y fin conocidos); `duracionMinutos` se calcula siempre acá adentro a partir de los timestamps, nunca se recibe como parámetro. `origen`: `"timer"` (Parte 1) | `"pomodoro"` (Parte 2) | `"manual"` (Parte 3, registro retroactivo — cuenta exactamente igual que una sesión en vivo para meta y felicitación, sin ninguna validación anti-trampa, pedido explícito).
...
Depende de: core/storage.js, core/storage-sync.js, core/utils.js, core/schema.js (`COLOR_TIEMPO_ESTUDIO_DEFAULT`), ui/componentes.js, ui/buscar-materia.js, semestres/semestres.js, main.js, tiempo-estudio/tiempo-estudio-config.js, tiempo-estudio/tiempo-estudio-timer.js, tiempo-estudio/tiempo-estudio-registro.js (Parte 3), tiempo-estudio/tiempo-estudio-estadisticas.js (Parte 3)
Exporta:
...
* `obtenerMateriasParaTiempoEstudio()` — **(exportada desde Parte 3)** recorre las materias matriculadas de los semestres actuales y resuelve nombre/plan de cada una; ya existía como helper privado, se exporta para que `tiempo-estudio-registro.js` (el `<select>` de materia del formulario) reciba la lista sin recalcularla ni crear un import circular de 3 puntas.
...
* `revisarFelicitacionMeta(materiaMatriculadaId)` — **(Parte 3)** revisa si la materia cruzó su meta semanal (sumando sus sesiones YA guardadas de la semana) y, si es así y todavía no se felicitó por la semana actual (`mm.tiempo_estudio.ultima_semana_felicitada`, epoch ms del lunes de la semana ya felicitada), muestra un overlay de felicitación (🎉, auto-cierra a los 3.5s o al tocar) y sella el campo para que no se repita en cada apertura de la app esa misma semana. Se llama SIEMPRE después de guardar la sesión que podría haber cruzado el umbral (nunca antes): desde `detenerTimerEstudio`, `avanzarFasePomodoro` (bloque de trabajo), `abrirAvisoSesionOlvidada`, y desde `tiempo-estudio-registro.js` tras un registro manual o una edición de sesión.
* `saltarDescansoPomodoro()` — **(2026-09-17, punto 1.4)** corta el descanso en curso y vuelve YA al bloque de trabajo siguiente. Hace exactamente la misma transición que la rama "descanso → trabajo" de `avanzarFasePomodoro` (incluido el conteo de bloques y el reinicio del ciclo tras un descanso largo), solo que sin esperar a que venza el reloj. Si estaba pausado a mitad del descanso, se reanuda solo (corriendo `sesionInicio` como hace `reanudarTimerEstudio`). Nunca genera sesión de estudio: un descanso no suma, se salte o no. `false` sin hacer nada si no hay timer, si no es Pomodoro, o si ya está en fase de trabajo. Lo consume el botón "⏭ Saltar descanso" de `construirPantallaDetalle` (tiempo-estudio.js), visible SOLO durante un descanso de esa misma materia.
* `pausarTimerEstudio()` / `reanudarTimerEstudio()` — **(2026-09-07, pedido explícito: "botón tipo play/pause y botón de detener sesión aparte")** pausan/reanudan el timer activo (simple o Pomodoro) SIN cerrar la sesión — eso lo sigue haciendo únicamente `detenerTimerEstudio`. Mecánica: al pausar se guarda `msPausaInicio`; al reanudar, `inicioFase` y `sesionInicio` se corren hacia adelante exactamente la duración de la pausa, así ningún otro cálculo del archivo (tick, `revisarMetaSimple`, `avanzarFasePomodoro`, `detenerTimerEstudio`, el salvavidas) necesita saber que existió una pausa — es como si el tiempo pausado nunca hubiera pasado. Mientras `pausado === true`, `tick()` no avanza fases de Pomodoro ni revisa la meta simple, y `segundosTranscurridos()` se congela en el instante de la pausa en vez de seguir con `Date.now()`. `timerEstaPausado()` — helper de lectura del estado de pausa del timer activo.
...
* `abrirModalEditarSesion(sesion, refrescar)` *(interna)* — a diferencia del registro manual (que pide fecha única + duración), este modal pide fecha+hora de INICIO y fecha+hora de FIN por separado — es lo que permite editar una sesión que cruza medianoche o varios días (ej. "empezaste 10pm, terminaste 2am") sin ningún truco especial: `fin` simplemente cae en otro día calendario, la resta de timestamps no le importa. Muta el objeto `sesion` in-place (ya vive en `estado.datos.sesiones_estudio`), re-sella su timestamp (`sellarTimestamp`) y llama a `revisarFelicitacionMeta()` por si el ajuste cruzó la meta.
...
Depende de: core/storage.js, core/schema.js (`sellarTimestamp`), core/storage-sync.js (`marcarCambioPendiente`), core/auth.js (`URL_WORKER_OAUTH`), ui/componentes.js (`mostrarToast`, `abrirConfirmacion`), core/clipboard.js (`copiarAlPortapapelesBlindado`, `abrirModalCopiaManualPortapapeles`), tiempo-estudio/tiempo-estudio-estadisticas.js (`calcularMinutosTotalesEnRango`, `obtenerRangoSemana`, ambas exportadas recién en esta ronda para esto)
Exporta:
* `construirVistaCompetencias(cont, refrescar)` — punto de entrada, llamado desde `tiempo-estudio.js` cuando el pill superior (ahora de 3, no 2) está en "Competencias". Cada tarjeta pinta primero lo que ya tiene en memoria (nombre, apodo, 👑 si `es_creador`) y carga el marcador en vivo (`GET /competencias/:id`) de forma asincrónica DENTRO de la tarjeta (`cargarMarcadorEnTarjeta`) — no bloquea el render de la lista esperando a todas las competencias a la vez. Ordenado por horas (ya viene ordenado del Worker), con medallas 🥇🥈🥉 y resaltado en negrita la fila propia. Botones por tarjeta: copiar invitación, 🏆 ver historial (`abrirModalHistorial`, `GET .../historial`), 🚪 salir (todos), 💥 borrar para todos (solo `es_creador`, con `token_creador`).
...
* **Exports nuevos (uso interno del grupo, no API de la sección):** `construirCajaModal`, `fetchConTimeout`, `formatearHoras`, `leerTokenCreador`, `salirDeCompetencia`, `borrarCompetenciaEntera` — los consume `tiempo-estudio-competencias-gestion.js`. Import circular intencional con ese archivo (allá se importa `abrirModalGestionCompetencia`/`construirRegistroCompetencias` de vuelta), seguro porque ningún nombre importado se usa en el nivel superior.
...
Depende de: core/storage.js, core/auth.js (`URL_WORKER_OAUTH`). Duplica `fetchConTimeout` (10 líneas) en vez de importarlo de tiempo-estudio-competencias.js, para no crear un ciclo — ese archivo sí importa de este.
Exporta:
* `mostrarCelebracionResultado(resultado)` — overlay a pantalla completa. `resultado` = `{ tipo: "victoria"|"derrota", nombreCompetencia, apodoGanador, horas, esPrueba }`. Confeti en `<canvas>` sin ninguna librería (mismo criterio que las gráficas SVG a mano de Estadísticas): victoria = estallido hacia arriba con colores vivos y mucha rotación; derrota = lluvia lenta y apagada que no para. Se le suma la animación de entrada de la tarjeta por CSS (`te-celebracion-entrada`, con variante suave para derrota) y el emoji con latido. Al cerrar ("Listo", único botón — no se cierra tocando el fondo) se corta el `requestAnimationFrame` Y el audio.
* `construirAvisosResultados(cont, refrescar)` — dibuja los avisos pendientes arriba de la lista de competencias. Los de prueba se pintan sincrónicos (ya están en memoria); los reales se agregan después, sin bloquear el render, con un `GET /historial` por competencia unida (mismo criterio asincrónico que `cargarMarcadorEnTarjeta`). Si el Worker no responde, no hay aviso y listo — no se le muestra ningún error al usuario por esto.
* `construirBotonesSimulacion(cont, refrescar)` — **BOTONES TEMPORALES DE PRUEBA** (punto 3.1), marcados como tales de 3 formas para que nadie los confunda con funcionalidad real: estilo *outline* punteado naranja propio (no usa `.btn` del resto de la app), la etiqueta "(prueba)" en el texto, y el comentario `// BOTÓN TEMPORAL DE PRUEBA - remover cuando el diseño de celebración esté aprobado` en cada punto del código. **Respetan el punto 3.3 igual que un resultado real**: no abren la celebración de una, encolan un aviso — así se prueba el flujo completo (aviso + celebración), no solo el confeti.
* `simularResultado(tipo, nombreCompetencia)` — encola el resultado falso. La cola (`resultadosDePrueba`) vive en memoria a nivel de módulo, no en `localStorage` ni en `estado.datos`: sobrevive a un re-render de la sección pero se va sola al recargar la página, que es lo que se quiere de algo temporal.
...
* `construirRegistroCompetencias(cont, refrescar)` — **(Parte 5)** sección "Registro de competencias", **colapsada por default** (`registroAbierto`, módulo-nivel, no persistida — mismo criterio que `vistaSeccionTE`). Una tarjeta chica por cada competencia `finalizada`, con solo su nombre. No dibuja nada si no hay ninguna finalizada.
...
* **`SECCIONES_TOGGLEABLES` (2026-09-09, pedido explícito) — se sumó `{ id: "tiempo-estudio", etiqueta: "Tiempo de Estudio", icono: "⏱️" }`.** Sin gate de existencia (a diferencia de "asistente"): Tiempo de Estudio ya existe siempre para cualquier usuario, así que el switch está disponible desde ya. **Verificación pendiente, sin `main.js`/`index.html` a la vista:** el `id` tiene que coincidir EXACTO con el `data-seccion` real del botón de nav de Tiempo de Estudio (se asumió `"tiempo-estudio"`, mismo criterio kebab-case que `"plan-estudios"`) y, si `DEFAULT_ORDEN_NAV` (main.js) no se autocompleta con ids nuevos de `SECCIONES_TOGGLEABLES` que todavía no estén guardados en `navegacion_orden` (el comentario de `reordenarSeccionNav` sugiere que sí, vía `obtenerOrdenNavegacionEfectivo`, mismo mecanismo que ya hizo aparecer "asistente" en su momento — pero no se pudo confirmar leyendo el código real de esa función), puede hacer falta agregarlo también a mano ahí.
