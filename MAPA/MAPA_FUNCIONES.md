...
* `MENSAJE_BIENVENIDA_WAPPER` + `mostrarSaludoInicial()` — texto fijo de bienvenida ("Soy Wapper…") más una fila de chips tocables (`crearChipEjemplo`) con las 12 plantillas de `PLANTILLAS_EJEMPLOS_BIENVENIDA_WAPPER`, cada `{materia}` resuelto por `construirEjemplosBienvenida()`.
